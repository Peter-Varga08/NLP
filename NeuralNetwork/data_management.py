...
def load_dataframe(_set='train', mode='full', exclude_modality=None, only_numeric=False, verbose=False):
    df = load_dataset("GroNLP/ik-nlp-22_pestyle", mode, data_dir="IK_NLP_22_PESTYLE")[_set].to_pandas()
...
def get_ling_feats():
    ...
    columns = set(list(df_mt.columns)) & set(list(df_tgt.columns)) & set(list(df_mt_test.columns)) & set(
        list(df_tgt_test.columns))
    df_mt = df_mt[columns]
